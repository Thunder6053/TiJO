class TimeOfTransfer:
    def __init__(self, hour, minute):
        self.hour = hour
        self.minute = minute

    def __str__(self):
        return f"{self.hour:02d}:{self.minute:02d}"

class Transfer:
    def __init__(self, amount, transfer_time):
        self._amount = amount
        # Tworzymy kopię defensywną obiektu TimeOfTransfer
        self._transfer_time = TimeOfTransfer(transfer_time.hour, transfer_time.minute)

    def get_transfer_time(self):
        # Zwracamy nową kopię obiektu, aby uniemożliwić modyfikację wewnętrznego stanu
        return TimeOfTransfer(self._transfer_time.hour, self._transfer_time.minute)

    def get_amount(self):
        return self._amount

    def execute_transfer(self):
        print(f"Wykonuje przelew na kwote {self._amount} o godzinie {self._transfer_time}")

scheduled_time = TimeOfTransfer(14, 30)
my_transfer = Transfer(100.00, scheduled_time)

print(f"Poczatkowy czas przelewu: {my_transfer.get_transfer_time()}")
my_transfer.execute_transfer()

# Próba ingerencji w czas przelewu...
time_from_getter = my_transfer.get_transfer_time()
time_from_getter.hour = 16
time_from_getter.minute = 0

print(f"\nCzas przelewu PO PRÓBIE ZEWNETRZNEJ INGERENCJI: {my_transfer.get_transfer_time()}")
my_transfer.execute_transfer()

